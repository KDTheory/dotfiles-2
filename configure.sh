...
source /dev/stdin <<< "$(curl -fsSL https://raw.githubusercontent.com/KDTheory/dotfiles/mac/script/prompt)"
...
echo ''
read -p "Do you want to use KDTheory's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down KDTheory dotfiles..."
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with KDTheory's macOS dotfiles..."
    else
        echo "KDTheory's macOS dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply KDTheory's macOS dotfiles. You will need to configure your environment manually..."
	echo ''
